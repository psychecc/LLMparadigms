...
import { byId, PARADIGMS } from '../paradigms.js'
...
const DEFAULT_COMPETITORS = [
  { name: 'ChatGPT', paradigm: 'AI 聊天助手', capability: '通用对话、工具调用、多模态输入', diff: '强通用能力，弱垂直场景闭环', call: '对话式直接调用 + 工具调用' },
  { name: 'Character.AI', paradigm: 'AI 陪伴', capability: '角色人格、长对话、关系沉浸', diff: '强情感粘性，商业化依赖关系深度', call: '角色 system prompt + 记忆召回' },
  { name: 'GitHub Copilot', paradigm: 'AI 副驾驶', capability: '代码补全、上下文建议、IDE 嵌入', diff: '深嵌工作流，迁移成本低', call: '编辑器上下文注入 + 流式生成' },
]

function competitorRows(extra, fallbackName) {
  const rows = [...DEFAULT_COMPETITORS]
  const r = extra?.report
  if (r) {
    rows.unshift({
      name: extra.name || fallbackName || '自定义竞品',
      paradigm: r.paradigms?.map((pp) => byId(pp.id)?.title || pp.id).join(' / ') || '待判断',
      capability: r.highlights?.slice(0, 2).join('；') || r.overview,
      diff: r.insights?.[0] || r.cons?.[0] || 'AI 已补全，建议继续看完整竞品分析页',
      call: r.paradigms?.[0]?.reason || '按产品场景调用 LLM',
    })
  }
  return rows
}
        ...
        <div className="flow-step">
          ...
          <div className="flow-map-panel">
            <div className="flow-section-head">
              <h4>范式地图模块</h4>
              <span>toC 7 种 + toB 7 种，点击卡片展开方法论摘要</span>
            </div>
            <div className="flow-paradigm-grid">
              {PARADIGMS.map((pa) => (
                <details className="flow-para-card" key={pa.id} style={{ '--accent': pa.accent }}>
                  <summary>
                    <span className="flow-para-n">{pa.n}</span>
                    <strong>{pa.title}</strong>
                    <em>{pa.role || pa.tag}</em>
                  </summary>
                  <p>{pa.method || GUIDE[pa.id]?.design?.[0] || pa.blurb}</p>
                  <div className="flow-para-actions">
                    <Link to={`/p/${pa.id}`}>可玩 Demo</Link>
                    <Link to={`/guide/${pa.id}`}>方法论</Link>
                  </div>
                </details>
              ))}
            </div>
          ...
          <div className="flow-cmp-box">
            ...
            <div className="flow-table-wrap">
              <table className="flow-compete-table">
                <thead>
                  <tr>
                    <th>竞品名称</th>
                    <th>范式归属</th>
                ...
                <tbody>
                  {competitorRows(cRes, cName).map((row, i) => (
                    <tr key={`${row.name}-${i}`}>
                      <td>{row.name}</td>
                      <td>{row.paradigm}</td>
      ...
      <div className="flow-stepper">
        {STEPS.map((s) => (
          <button key={s.n} className={`flow-pip ${step === s.n ? 'on' : ''} ${done[s.n] ? 'done' : ''}`} onClick={() => setStep(s.n)}>
            <span className="flow-pip-n">{s.n}</span>
