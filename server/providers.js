...
export function resolve(b = {}) {
  ...
  let model = b.model || DEFAULT_MODEL[provider]
  if (provider === 'deepseek' && LEGACY_DEEPSEEK_MODEL[model]) model = LEGACY_DEEPSEEK_MODEL[model]
...
function deepseekClient(b = {}) {
  ...
  if (!apiKey) {
    throw new Error('未配置 DeepSeek API Key —— 点顶部右侧「模型设置」填入，或设置环境变量 DEEPSEEK_API_KEY')
  }
